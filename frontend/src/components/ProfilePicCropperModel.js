...
const ProfilePicCropperModel = ({ heading, active, setActive, profilePicSrc, setProfilePicSrc, setCroppedImageBase64, clearFileInputOnCancel, setCurrentUser }) => {
    ...
    return (
        <Modal heading={heading} active={active} setActive={closeModelAndClearFileInput}>
            <div className="profile-pic--cropper-container" style={{height: '90%', overflow: 'hidden'}}>
                <ReactCrop src={profilePicSrc} crop={crop} onChange={newCrop => setCrop(newCrop)} onImageLoaded={setImage} />
            </div>
            <div className="modal-actions">
                <button className="btn btn-1 btn-md" style={{ marginRight: '8px' }} onClick={getCroppedImage}>
                Crop & Update
                </button>
                <button className="btn btn-1 btn-md" onClick={() => closeModelAndClearFileInput(false)}>
                Cancel
                </button>
            </div>
        </Modal>
